...
if NETWORK_DETECTION_ENABLED and CONFIG_RED:
    ...
    logger.info(
        f"✅ ALLOWED_HOSTS configurado automaticamente para red: {CONFIG_RED['nombre']}"
    )
...
# ✅ NUEVO: Configuración para desarrollo / producción
if DEBUG:
    SIMPLE_JWT["ACCESS_TOKEN_LIFETIME"] = timedelta(hours=24)  # 24 horas en dev
    logger.info("🔧 JWT: Modo DEBUG - Tokens de 24 horas")
else:
    SIMPLE_JWT["ACCESS_TOKEN_LIFETIME"] = timedelta(hours=2)  # 2 horas en prod
...
if not DEBUG:
    if NETWORK_DETECTION_ENABLED and CONFIG_RED:
        ...
        logger.info(
            f"✅ CORS_ALLOWED_ORIGINS: {len(CORS_ALLOWED_ORIGINS)} origenes en produccion"
        )
    else:
        CORS_ALLOWED_ORIGINS = get_env_list(
            "CORS_ALLOWED_ORIGINS", "https://api.jpexpress.com,https://jpexpress.com"
        )
else:
    # 🔥 MODO DEBUG → ORÍGENES POR DEFECTO PARA REACT (5173) + BACKEND (8000)
    CORS_ALLOWED_ORIGINS = get_env_list(
        "CORS_ALLOWED_ORIGINS",
        "http://localhost:5173,http://127.0.0.1:5173,http://172.16.60.5:5173,http://192.168.1.5:5173,"
...
if NETWORK_DETECTION_ENABLED and CONFIG_RED:
    ...
    logger.info(
        f"✅ CSRF_TRUSTED_ORIGINS: {len(CSRF_TRUSTED_ORIGINS)} origenes configurados"
    )
else:
    # 🔥 Incluimos también el puerto 5173 (Vite) además de 8000
    CSRF_TRUSTED_ORIGINS = get_env_list(
        "CSRF_TRUSTED_ORIGINS",
        "http://localhost:8000,http://127.0.0.1:8000,"
        "http://localhost:5173,http://127.0.0.1:5173,"
        "http://172.16.60.5:5173,http://192.168.1.5:5173",
    )
...
def log_startup_info():
    ...
    if NETWORK_DETECTION_ENABLED and CONFIG_RED:
        print(
            f'✅ Red detectada: {CONFIG_RED["nombre"]} ({CONFIG_RED.get("modo", "AUTO")})'
        )
