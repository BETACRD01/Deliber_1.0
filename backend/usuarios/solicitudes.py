...
class ValidadorSolicitudCambioRol:
    ...
    def validar_usuario_puede_solicitar(usuario):
        ...
        if not usuario.email:
            raise ValidationError({"usuario": "Completa tu email en el perfil"})

        if not usuario.celular:
            raise ValidationError({"usuario": "Completa tu teléfono en el perfil"})
    ...
    def validar_rol_solicitado(usuario, rol_solicitado):
        ...
        solicitud_pendiente = SolicitudCambioRol.objects.filter(
            user=usuario, rol_solicitado=rol_solicitado, estado="PENDIENTE"
        ).exists()
        ...
        ultimo_rechazo = (
            SolicitudCambioRol.objects.filter(
...
class GestorSolicitudCambioRol:
    ...
    def aceptar_solicitud(cls, solicitud, admin, motivo_respuesta=""):
        """
        ✅ MODIFICADO: Ahora guarda el rol anterior antes de cambiar
        """
        if solicitud.estado != "PENDIENTE":
            raise ValidationError(
                f"La solicitud ya fue {solicitud.get_estado_display().lower()}"
            )

        # ✅ GUARDAR ROL ANTERIOR
        solicitud.rol_anterior = solicitud.user.rol

        # Cambiar el rol del usuario
        solicitud.user.rol = solicitud.rol_solicitado
        solicitud.user.save(update_fields=["rol", "updated_at"])

        # Actualizar la solicitud
        solicitud.estado = "ACEPTADA"
        solicitud.admin_responsable = admin
        solicitud.motivo_respuesta = motivo_respuesta or "Solicitud aceptada"
        ...
        logger.info(
            ...
            f"{solicitud.rol_anterior} → {solicitud.rol_solicitado} "
            f"por {admin.email}"
        )
    ...
    def rechazar_solicitud(solicitud, admin, motivo_respuesta):
        ...
        Args:
            ...
            motivo_respuesta (str): Motivo del rechazo (opcional)
        ...
        try:
            ...
            solicitud.admin_responsable = admin
            solicitud.motivo_respuesta = motivo_respuesta or ""
            solicitud.respondido_en = timezone.now()
            solicitud.save(
                update_fields=[
                    "estado",
    ...
    @staticmethod
    def _crear_proveedor(solicitud):
        """
        ✅ CORREGIDO: Crea registro de Proveedor cuando es aceptado
        Ahora recibe la solicitud completa, no solo el usuario
        """
        try:
            ...
            if Proveedor.objects.filter(user=usuario).exists():
                logger.info(f"⚠️ Proveedor ya existe para {usuario.email}")
                return

            Proveedor.objects.create(
                user=usuario,
                nombre=usuario.get_full_name() or usuario.email,
                ruc=getattr(solicitud, "ruc", "TEMP" + str(usuario.id).zfill(10)),
                email=usuario.email,
    ...
    @staticmethod
    def _crear_repartidor(solicitud):
        """
        ✅ CORREGIDO: Crea registro de Repartidor cuando es aceptado
        Ahora recibe la solicitud completa, no solo el usuario
        """
        try:
            ...
            # Verificar que no exista ya
            if Repartidor.objects.filter(user=usuario).exists():
                logger.info(f"⚠️ Repartidor ya existe para {usuario.email}")
                return

            Repartidor.objects.create(
                user=usuario,
                cedula=getattr(solicitud, "cedula_identidad", usuario.celular),
                telefono=usuario.celular,
    ...
    def _registrar_auditoria(
        admin, tipo_accion, descripcion, solicitud=None, request=None
    ):
        """
        ✅ CORREGIDO: Registra la acción en auditoría con manejo de errores
        """
        try:
            ...
            # Si no hay request, usar valores por defecto
            if not user_agent:
            ...
            logger.info(f"✅ Auditoría registrada: {tipo_accion}")
        except Exception as e:
            logger.error(f"⚠️ Error registrando auditoría: {e}", exc_info=True)
    ...
    def notificar_solicitud_creada(solicitud):
        ...
        try:
            ...
            if not perfil.puede_recibir_notificaciones:
                logger.info(
                    f"⚠️ Usuario {solicitud.user.email} no tiene notificaciones habilitadas"
                )
                return
            ...
            logger.info(
                f"📬 Notificación FCM enviada a {solicitud.user.email}: {mensaje}"
            )
