...
class CrearDireccionSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if not etiqueta or not str(etiqueta).strip():
            ...
            logger.info(f"📝 Etiqueta generada automáticamente: '{etiqueta}' para {user.email}")
        else:
    ...
    def _generar_etiqueta_unica(self, user):
        ...
        logger.info(f"📋 Etiquetas existentes para {user.email}: {etiquetas_existentes}")
        ...
        for numero in range(1, 101):
            ...
            if etiqueta_propuesta not in etiquetas_existentes:
                logger.info(f"✅ Etiqueta disponible encontrada: '{etiqueta_propuesta}'")
...
class ActualizarDireccionSerializer(serializers.ModelSerializer):
    ...
    def validate_etiqueta(self, value):
        ...
        if len(value) < 2:
            raise serializers.ValidationError("La etiqueta debe tener al menos 2 caracteres")

        if len(value) > 50:
            raise serializers.ValidationError("La etiqueta no puede exceder 50 caracteres")
    ...
    def validate(self, data):
        ...
        if 'etiqueta' in data:
            ...
            if not etiqueta or not str(etiqueta).strip():
                ...
                logger.info(f"📝 Manteniendo etiqueta existente: '{self.instance.etiqueta}'")
